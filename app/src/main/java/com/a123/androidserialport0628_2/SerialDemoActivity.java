...
public class SerialDemoActivity extends AppCompatActivity {
    ...
    private TextView tv_ttymxc4_send;
    private TextView tv_ttymxc4_receive;
    ...
    static class MyHandler extends Handler {
        ...
        MyHandler(SerialDemoActivity activity) {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        TextView tv_port = findViewById(R.id.id_tv);

        String[] devices = new SerialPortFinder().getAllDevicesPath();
        // 串口设备
        if (devices.length > 0) {
            String device = PreferenceUtils.getInstance().getDevice();
            tv_port.setText(device.equals("") ? devices[0] : device);
            try {
                mSerialPort = new SerialPort(new File(device), 9600, 0);//这里串口地址和比特率记得改成你板子要求的值。
